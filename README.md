...
- 몬스터볼이 **컬러 그라데이션**으로 좌우 역동적으로 흔들립니다
- 주변에 **별빛 파티클**(✦ ✧ ·)이 떠다닙니다
- 완료되면 중앙이 ✨로 바뀌며 **"딸깍!"** + 파티클이 터집니다
...
- **Python 3.10~3.11** (3.12+는 일부 패키지 호환 문제)
- **ffmpeg** (오디오 변환에 필요)

### 설치 방법 (uv 사용 — 권장)

[uv](https://docs.astral.sh/uv/)를 사용하면 Python 버전 관리와 의존성 설치를 한 번에 할 수 있습니다.
...
# 4. 가상환경 생성 (Python 3.11 필수 — 3.12+는 일부 패키지 호환 문제)
uv venv --python 3.11
...
# 6. 실행
uv run python transcribe.py lecture.m4a
```
...
# 3. 가상환경 생성 및 활성화 (Python 3.10~3.11)
python -m venv .venv
...
uv pip install anthropic   # 또는 pip install anthropic
...
입력(m4a/mp3/wav)
  ...
  ├─ 6. 음성 전사       Whisper large-v3-turbo / Qwen3-ASR-0.6B
...
│   ├── asr.py              # ASR 전사 엔진 (Whisper / Qwen3-ASR)
...
| 모드 | 피크 메모리 |
|------|-----------|
| Whisper + --asr-only | ~4GB |
| Whisper + 전처리 (--denoise light) | ~5GB |
| Qwen3-ASR + 전처리 | ~10GB+ |
